...
make_practice_page <-  function(page_no, img_dir) {
  psychTestR::reactive_page(function(answer, ...) {
    ...
    get_practice_page(page_no, feedback, img_dir)
  })
}



get_practice_page <- function(page_no, feedback, img_dir){
  key <- sprintf("PRACTICE%d", page_no)

  prompt <- psychTestR::i18n(key, html = T, sub = list(feedback = feedback))
  subprompt <- psychTestR::i18n("PRACTICE_SUBPROMPT", html = T, sub = list(feedback = feedback))
...
practice <- function(img_dir) {
  lapply(1:2, make_practice_page, img_dir) %>% unlist()
}
